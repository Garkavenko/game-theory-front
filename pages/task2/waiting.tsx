...
function Waiting() {
  ...
  const mutation = useMutation(`finishTask1Registration${token}`, () => {
    return axios('http://localhost:8080/task2/finishRegistration', {
      method: 'post',
      data: {
        token
      }
  ...
  const query = useQuery(`roomParticipantsInfo${token}`, () => {
    return axios(`http://localhost:8080/task2/roomParticipantsInfo?token=${router.query.token}`, {
      method: 'get',
    })
  }, {
      ...
      <Container style={{ backgroundColor: '#fff', height: '100%' }}>
        ...
        <Button onClick={() => {
          mutation.mutate(undefined, {
            onSuccess: () => {
              router.push(`/task2/playersSettings?token=${token}`)
            }
          });
